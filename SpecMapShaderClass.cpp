...
{
	return InitializeShader(device, hwnd, L"./hlsl/SpecmapVertexShader.hlsl", L"./hlsl/SpecmapPixelShader.hlsl");
}
...
bool SpecMapShaderClass::Render(ID3D11DeviceContext *deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix
	, int textureNum, ID3D11ShaderResourceView **textureArray, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
	if (!SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, textureNum, textureArray, lightDirection, diffuseColor, cameraPosition, specularColor, specularPower))
	{
		return false;
	}
...
HRESULT SpecMapShaderClass::InitializeLayout(ID3D11Device *device, ID3D10Blob *vertexShaderBuffer)
{
